...
- **Edges** (`sst/edges/`, native CSR): two files per `(edge_type, level)` — forward (`edges-fwd`, keyed by `src_id`) and inverse (`edges-inv`, keyed by `dst_id`) — sharing one wire format distinguished by `FLAG_INVERSE_PARTNER`, giving O(deg(v)) expansion in both directions. Exact `(src,dst)` probes first binary-search the endpoint key and then the partner inside its independently addressable block: dense UUID blocks are fixed-width and use binary search, while split-top64/bottom64 blocks stop as soon as the sorted target range is passed. Once an SST reader is open, this makes bound-endpoint `MERGE`/Expand-Into proportional to the overlapping SST generations and logarithmic in dense degree rather than decoding and filtering every relationship of the endpoint; existence-only probes do not decode property streams. A cold object-store miss still fetches the complete immutable CSR object and opens its topology, so this is a CPU/decoded-working-set point seek rather than ranged remote I/O. 64-byte frozen header (`HEADER_MAGIC = "TGEDGE\0\0"`, `format_major=1`, `format_minor=0`, `flags`, and `blake3(edge_type/src_label/dst_label)[..16]`). Independently addressable sections: `key_ids` (0x0001), `offsets` (0x0002, bitpacked u24/u32/u40/u48), `partners` (0x0003, per-group split-top64/bottom64 `TAG_SPLIT=0x01` or dense `TAG_DENSE=0x10`), `per_edge_lsn` (0x0004), `per_edge_tombstones` (0x0005, omitted when none), `fence_index` (0x0006, emitted when `key_count > FENCE_INDEX_THRESHOLD = 65 536`, stride `DEFAULT_FENCE_STRIDE = 256`), and per-property streams (0x0100, Zstd Arrow IPC). Footer: section table + body fields + a fixed 20-byte trailer (`FOOTER_TRAILER_LEN`) = xxh3-64(body) ‖ footer_len:u32 ‖ `FOOTER_MAGIC = "TGEDGE\xFE\xEF"`.
