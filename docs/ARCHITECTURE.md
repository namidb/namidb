...
From these two primitives the engine derives a monotonic commit log (a Create-only pointer family), single-writer mutual exclusion (epoch fencing), snapshot-isolated reads, and safe concurrent space reclamation. This lineage — monotonic commit files plus a writer epoch — is shared with systems such as Delta Lake and SlateDB; NamiDB's contribution is to carry it through a *graph* data model with columnar adjacency, an in-graph approximate-nearest-neighbour index, and a full-text index, while preserving snapshot freshness across every indexed path. Full-text and property indexes remain exact; ANN membership is approximate by design, while freshness gates, exact candidate scoring, and flat fallbacks prevent stale rows and semantic drift.
...
- **Index freshness and semantic parity.** A secondary index may never serve a row from an older logical corpus. Two mechanisms enforce this: unioning the in-memory delta the index has not yet absorbed, or gating on a log-sequence-number comparison and falling back to the exact scan (§4, §6, §7). Full-text and property-index answers are scan-equivalent. Vector results use Vamana approximate navigation, then exact f32 scoring over the reached candidate pool; the approximation is recall (which neighbours are reached), not snapshot visibility or score semantics.
...
`SstKind` = `Nodes | EdgesFwd | EdgesInv | VectorGraph | TextIndex`. `flush.rs` builds immutable bodies on Tokio's blocking pool behind one process-wide single-flight permit. The permit is moved into the blocking closure, so cancelling its async waiter cannot let a retry overlap the detached corpus-sized build. Flush admits at most four independent object PUTs at once, waits for every sibling even after an error, then commits one manifest that appends the new `SstDescriptor`s and `clears wal_segments` (their records are now durable in SSTs). Large objects use at most eight concurrent 5 MiB multipart parts; a cancellation guard aborts an unfinished upload if its task is dropped. Node Parquet and exact-record output are written to anonymous disk spools, synchronised before delayed-allocation errors can cross the publish boundary, and exposed to the existing upload path through an immutable mmap or streaming file rather than copied into corpus-sized heap buffers. Each descriptor carries embedded stats (`min_key/max_key` base64, `min_lsn/max_lsn`, `row_count`, `KindSpecificStats`, per-label/property stats) so the read path prunes candidates with zero extra GETs, plus optional side-car pointers (bloom, unique/equality/label indices and the node locator).
...
- **Exact node locator** (`.nloc2`, `sst/paged_index.rs`): two fixed-page `NAMIPG02` B+trees concatenated as `[NodeId → physical row ordinal][NodeId → exact binary record]`. The first tree is the complete body understood by 2.0.4/2.0.5 readers; trailing bytes are therefore rollback-safe. Current point read-modify-write probes range-read only the headers, search-path pages and matching compressed `{lsn, operation, node payload}` record, avoiding the unrelated wide `__overflow_json` Parquet page. B+tree pages stay compact in memory while the variable-size exact-record value region streams to an anonymous file and then directly into multipart upload.
- **Edges** (`sst/edges/`, native CSR): two files per `(edge_type, level)` — forward (`edges-fwd`, keyed by `src_id`) and inverse (`edges-inv`, keyed by `dst_id`) — sharing one wire format distinguished by `FLAG_INVERSE_PARTNER`, giving O(deg(v)) expansion in both directions. Current forward SSTs optionally add a fixed-page `.epidx` B+tree from `(src_id,dst_id)` to the checksummed winning `{LSN,tombstone,properties}` record. Bound-endpoint `MERGE`/Expand-Into groups every correlated pair into one range-readable probe per overlapping SST, so a cold object-store lookup does not fetch or retain the complete graph-sized CSR. The sidecar is an accelerator only: all-or-nothing build limits, absence, corruption or a 2.0.4 janitor sweep select the exact CSR fallback. That fallback binary-searches the endpoint key and partner inside its independently addressable block: dense UUID blocks are fixed-width, while split-top64/bottom64 blocks stop as soon as the sorted target range is passed; existence-only probes avoid property streams. The authoritative CSR has a 64-byte frozen header (`HEADER_MAGIC = "TGEDGE\0\0"`, `format_major=1`, `format_minor=0`, `flags`, and `blake3(edge_type/src_label/dst_label)[..16]`). Independently addressable sections: `key_ids` (0x0001), `offsets` (0x0002, bitpacked u24/u32/u40/u48), `partners` (0x0003, per-group split-top64/bottom64 `TAG_SPLIT=0x01` or dense `TAG_DENSE=0x10`), `per_edge_lsn` (0x0004), `per_edge_tombstones` (0x0005, omitted when none), `fence_index` (0x0006, emitted when `key_count > FENCE_INDEX_THRESHOLD = 65 536`, stride `DEFAULT_FENCE_STRIDE = 256`), and per-property streams (0x0100, Zstd Arrow IPC). Footer: section table + body fields + a fixed 20-byte trailer (`FOOTER_TRAILER_LEN`) = xxh3-64(body) ‖ footer_len:u32 ‖ `FOOTER_MAGIC = "TGEDGE\xFE\xEF"`.
...
- **VectorGraph** (`.vg`, `sst/vector.rs`, `vector-index` feature): 8-byte magic `"NAMIVG04"` + bincode `VectorGraphBody` (DiskANN/Vamana graph, f32 or int8 vectors, bounded adaptive metadata postings). Exact `"NAMIVG03"` bodies remain readable without native postings; unknown/corrupt bodies fall back to flat scan.
...
The same `gc` predicate gates **index rebuilds**. A Vamana vector graph or BM25 text index is not row-mergeable, so compaction rebuilds it from the merged corpus only when its durable `search_index_builds` generation is stale. The `NodeMergeIndexSpecs` are populated (from `manifest.vector_indexes` / `text_indexes`) only when `gc` holds and a rebuild is required; on a partial merge the member lists stay empty, the existing `.vg`/`.ft` is left untouched, and the freshness gate `Snapshot::index_outrun_by_nodes` routes reads to the exact flat scan until an authoritative merge rebuilds. A settled 2.0.5 node SST whose logical corpus is already covered takes a physical-only migration path: compaction attaches a fresh `.nloc2` sidecar while preserving the Parquet descriptor, vector/text bodies, object IDs and build generations. Treating either that migration or a per-bucket deepest merge in a mixed-scope namespace as a new corpus would wastefully rebuild — or permanently truncate — search, so logical generation and physical acceleration are tracked separately.

### Prepare / commit split

A sweep is split so the single writer lock is held only for the cheap manifest CAS. `WriterSession::compaction_basis()` (`ingest.rs`) clones a `CompactionBasis{ manifest_store, fence, base }` under the lock (two Arc-backed clones plus a manifest clone). `CompactionBasis::prepare` → `prepare_leveled` then runs **off-lock**: it plans every bucket, maps local file-store inputs directly, streams every remote input — including individually small L0 objects — into an anonymous file, forces writeback with `sync_data`, and mmaps it, performs the k-way merges and any required vector/text index rebuilds on the blocking pool (`run_cpu` via `spawn_blocking`), and PUTs every output body, bloom, and sidecar at immutable UUID-v7-derived paths (`{uuid.simple}-{tag}-{scope}.{parquet|csr|vg|ft}`). Node Parquet output follows the same synchronised spool-then-mmap path and exact-record values remain synchronised and spooled through upload, so the corpus-sized node input/output byte streams are file-backed rather than heap-resident (an actually stale vector/text generation still requires its rebuild members in memory). Spool placement is controlled by `NAMIDB_SPOOL_DIR` (disk-backed `/var/tmp` on bare Unix deployments; `/var/tmp/namidb-spool` in the official image). A full-backlog node pass temporarily needs `sum(inputs) + parquet_output + exact_record_output` on that filesystem — roughly three times the compacted live bytes, plus superseded-version headroom. The result `PreparedCompaction{ new_descs, removed_ids, bloom_count, base_version }` references no manifest.
...
`merge_node_sources` / `merge_edge_sources` never materialise the whole decoded bucket. Node inputs begin as file-backed mappings. `NodeSourceCursor::open` parses metadata but does not decode a row; sources remain in an unopened min-heap until their manifest `min_key` reaches the active winner frontier, and an activated source retains at most one 64-row Arrow batch (`NODE_MERGE_INPUT_BATCH_ROWS = 64`), never a complete row group. `EdgeSourceCursor` walks one partner block plus one IPC mini-batch per property stream, decoding zstd through a streaming `Read`. A `BinaryHeap<Reverse<HeapEntry>>` is keyed **(key asc, lsn desc, source order)** — for nodes `key = node_id`, for edges `key = (key_id, partner_id)`. Popping yields, per key, the highest-LSN observation first (its winner), exact ties broken toward the earlier source, reproducing the stable `sort_by(key, lsn desc)` a materialised merge would produce. The first pop of a key is the winner: it is materialised (for nodes, the JSON property-map re-encode, typically 3–10× the Parquet size, is paid **only** for winners) and pushed into an `IncrementalNodeSstWriter` in bounded chunks of `NAMIDB_COMPACTION_MERGE_CHUNK_ROWS` rows (default `NODE_SST_BATCH_ROWS = 16·1024 = 16384`). Shadowed duplicates and, when `gc`, winning tombstones are skipped without ever being converted (`cursor.advance()` / `pop(false)` still steps the property streams in lockstep). Complexity is O(N log k) for N total input rows across k sources. Streaming collectors observe the winner stream in one pass: `UniqueSidecarCollector`, `EqualitySidecarCollector`, `LabelIndexCollector`, `PerLabelStatsCollector` (RFC-025), and — only for a stale authoritative search generation — the `VectorMemberCollector`/`TextMemberCollector` member harvesters. **Residual memory per bucket** (by design): file-backed input/output mappings, one 64-row decoded batch per activated node source, one winner chunk, compact sidecar maps/pages and multipart windows, plus the embeddings/documents only when a Vamana/BM25 rebuild inherently needs the complete corpus.

### Space reclamation: the horizon-aware sweep

`sweep_orphans(manifest_store, retention_horizon, min_age, max_level, delete)` (`janitor.rs`) deletes only objects reachable by no live reader. The **retention horizon** is the oldest manifest version any live reader is pinned to: `SnapshotCell::retention_horizon()` returns `min(SnapshotRegistry::min_live(), current)`, where the registry is a `BTreeMap<version, refcount>` maintained by `PinnedSnapshot` acquire/drop (`read.rs`); it is unioned in the sweep with the pin-lease floor and clamped to `current_version`. The sweep then builds the **live set** as the union of objects referenced by every retained manifest version from `horizon` to `current` inclusive — SST bodies, bloom / unique / equality / label-index sidecars (all under the same `sst/level{N}/` prefix), and WAL seqs. It recursively lists the namespace's `sst/` prefix and accepts only canonical direct `sst/level{N}/<file>` objects; `max_level` is a diagnostic expectation, not a correctness boundary, so an output written to a deeper level before a failed compaction CAS is still reclaimed. Any listed object not in the live set and older than `min_age` is an orphan. It also reclaims `manifest/v{N}.json` and `manifest/pointer/p{N}.json` strictly below the horizon (RFC-029), dead WAL segments referenced by no retained version, and a stale `memtable_snapshot.bin` once `last_lsn ≤ horizon_flushed_hwm`. Safety is **by construction, not wall-clock**: an object the sweep deletes is referenced by no version at or above the horizon, so no reader can reach it; `min_age` (default 24 h) is only a secondary guard for the body-PUT-then-CAS race where a just-written object is not yet referenced by any committed version. The interaction with compaction is exact: compaction removes source descriptors from the manifest but leaves the bodies; a reader pinned at the pre-compaction version holds the horizon down, so those bodies stay live until it drops, then become reclaimable. The server schedules this after compaction only when `compaction_interval` is non-zero (default 300 s); `sweep_delete=true` (the default) performs deletion, while `false` is a dry run. The reactive trigger `compaction_l0_trigger` (default 8) compacts on flush when `max_l0_bucket_len() ≥ 8`, and a soft write stall (`write_stall_l0` default 24, `write_stall_delay` 50 ms) throttles the writer above three times the trigger.
...
(default `1073741824`, exact configured bytes) scales all active ceilings proportionally
...
estimated weight exceeds their effective tier capacity. The limit applies to
accounted entry weights; allocator/hash metadata and values held by an active
query after eviction are outside that residency gauge.
...
| `SstCache.search_indexes` | one weighted foyer pool shared by decoded `.ft` / `.vg` indexes | combined 512 + 512 MiB legacy request; optionally reserved with `NAMIDB_SEARCH_INDEX_CACHE_MAX_BYTES` | S3-FIFO + manifest pruning |
...
Vector and text bodies are monolithic in the current format, so a single
decoded index must fit as one cache entry. They share a pool rather than fixed
per-kind partitions: either kind may consume the whole assignment and evict
the other. `NAMIDB_SEARCH_INDEX_CACHE_MAX_BYTES`, when set, reserves that many
exact bytes *inside* `NAMIDB_CACHE_MAX_BYTES` before all remaining tiers are
scaled into the remainder. Preflight and insertion use the same conservative
deep estimate (`wire × 6`, plus corpus-shape floors), so a miss never decodes
successfully only to discover it cannot be retained. Capacity refusal is a
typed `CacheCapacity` error (HTTP 503 / Bolt transient) with required and
assigned bytes and increments
`namidb_search_index_cache_admission_rejections_total{kind}`. It is deliberately
not `Ok(None)`: only missing, stale, legacy, or corrupt optional accelerators
select the exact flat fallback.
...
**Multi-tenant sharing (docs/multi-tenancy.md).** One host serves N namespaces under one set of budgets. The `SstCache` is namespace-safe *by construction* — every key is an absolute object-store path (namespace-prefixed) or `(path, row-group)`. The `NodeViewCache` and `AdjacencyCache` are **not**: the bare `(generation, label/edge_type, …)` triple collides across tenants because per-namespace generations both start at 1. `NodeCacheKey` and `AdjacencyKey` therefore embed `namespace: Arc<str>` (the `<root>/<ns>` prefix, rendered once per snapshot as `cache_namespace` so per-key clones are pointer-cheap); tests `same_triple_different_namespace_is_a_distinct_slot` pin this. `retain_paths(namespace_prefix, live)` prunes side-map entries **scoped to one namespace** (normalised to a `/` boundary so `tenants/a` never claims `tenants/a2`) on each manifest commit — a global retain would evict every other tenant's warm state per flush. `prune_namespace` eagerly drops a namespace's entries from all three shared cache families on eviction. A decode begun before eviction can finish later, so the SST cache retains a bounded FIFO of at most 4096 deny-all namespace tombstones; canonical `<prefix>/sst/...` paths recover that prefix without allocation for O(1) admission. Reopening removes its tombstone, while churn cannot grow the guard indefinitely. A decode delayed beyond that bounded FIFO can at worst add an unreachable immutable entry inside the byte-capped cache; manifests reference exact UUID paths, so it cannot change query results.

### Index freshness and flat fallback

The invariant: an SST-backed secondary index (`.vg` DiskANN/Vamana vector, `.ft` BM25 text) must describe the same committed corpus as its flat path. BM25 is additionally score/ranking equivalent; Vamana remains approximate in candidate recall. Two mechanisms enforce corpus freshness. (1) **Union the memtable delta.** `vector_fresh_delta` (`read.rs`) returns every node id the `.vg` has not absorbed (committed memtable + staged overlay, highest-LSN-wins): `Some(embedding)` to merge into the KNN, `None` to *suppress* an id that is tombstoned, no longer carries the label, or dropped its embedding; the executor unions this with the ANN result. (2) **Gate and fall back.** `index_outrun_by_nodes(index_name, kind)` (`read.rs`) reports whether any persisted `Nodes` SST has `max_lsn` greater than the index's — comparing **LSNs, not levels**, which closes the partial-compaction window where a shallow merge rewrites a subset to L1 and empties L0. Exactly one immutable body must represent an index generation; zero or multiple `.vg`/`.ft` bodies select the flat path rather than serving stale vectors or combining BM25 scores computed with incompatible corpus statistics. The lockstep `Nodes` SST written by the same authoritative (deepest-level) rebuild shares the index's `max_lsn` exactly, so it is never flagged. `text_search` returns `Ok(None)` — "fall back to the exact flat scan" — when no single authoritative index body exists, when `index_outrun_by_nodes` is true, when an unflushed upsert carries the indexed label (a live corpus delta that would shift BM25's corpus-wide N/avgdl/df stats), or when a tombstoned/relabelled id is itself an indexed document. The label-scoped check means an unflushed write to an *unrelated* label no longer demotes every `search.bm25` to an O(corpus) scan.
...
`optimize::optimize(plan, catalog)` (`crates/namidb-query/src/optimize/mod.rs`) first runs the structural `apply_edge_count_pushdown` once, then iterates a rewrite battery to a fixpoint capped at `MAX_FIXPOINT_ROUNDS = 8` (each well-formed rewrite is idempotent within two rounds; the cap is defensive). Per round, in order: (1) `unique_lookup` collapses `Filter(prop = literal) → NodeByPropertyValue` when the property is schema-`unique` (point lookup) or non-unique-`indexed` (`multi:true` posting-list lookup), and `WHERE elementId(n)=…` → `NodeById`; (2) `anchor_inversion` re-anchors a single-hop, non-optional pattern at its selective endpoint: when the `Expand` reads from a bare `NodeScan` but the expand *target* carries an index-answerable equality, it swaps sides — `NodeByPropertyValue` on the target, direction flipped, residual predicates kept above (unique targets always; non-unique indexed targets only when stats say the target label is no larger than the source; var-length/OPTIONAL/back-reference/shortestPath/walker-materialized-trail shapes are refused); (3) `vector_search` (feature `vector-index`) rewrites a KNN shape to `VectorSearch`; (4) `predicate_pushdown` splits each `Filter` on `AND` and sinks each conjunct toward the leaves, folding pushable single-column conjuncts into `NodeScan.predicates` (RFC-013 Parquet row-group pruning) with `Distinct`/`TopN`/write operators acting as barriers; (5) `normalize_filters` merges adjacent filters and drops `Filter(true)` and redundant `__label_eq`; (6) `convert_cross_to_hash` (RFC-012); (7) `convert_semi_apply_to_hash_semi_join` (RFC-014 decorrelation); (8) `reorder_joins` (RFC-016); (9) `detect_multiway_join` (RFC-024); (10) `apply_projection_pushdown` (RFC-015) runs last so it sees the final `NodeScan` shape. Reaching the KNN→`VectorSearch` rewrite *below* `WITH`/`Project`/`Filter` chains matters because `vector_search::recurse` rebuilds those wrappers structurally and re-attempts the match at each parent. A source `WHERE` below `TopN` becomes the operator's pre-k residual filter; a `Filter` above `TopN` remains a separate post-k operator and may shorten the bounded page without refilling it.
...
NamiDB accelerates top-k nearest-neighbour retrieval over node embeddings with a DiskANN/Vamana approximate-nearest-neighbour (ANN) graph index, gated behind the `vector-index` Cargo feature and documented in RFC-030. Freshness gates, a memtable/overlay delta merge, adaptive over-fetch, and an exact flat-scan fallback guarantee that indexed search observes the current corpus and applies the same metric semantics as the flat path. Vamana navigation is still approximate: membership depends on recall and `ef`, while reached candidates are scored exactly from their stored f32 embeddings (except the explicitly quantized navigation representation). The algorithm layer (`crates/namidb-ann`) is storage-agnostic and generic over a `VectorSpace` trait; the storage layer (`crates/namidb-storage/src/sst/vector.rs`) wraps it in a self-contained `.vg` SST body; the query layer (`crates/namidb-query`) rewrites KNN-shaped Cypher into a `VectorSearch` operator and merges index results with fresh writes.
...
Because the graph navigates on the *quantized* cosine, served scores would otherwise drift from the flat scan. The executor closes this: for an int8 index, `try_index_search` fetches an over-fetch pool (`INT8_RESCORE_POOL = 4`, i.e. 4k candidates even with no filter) and **rescores every reached candidate with the exact f32 metric from its stored embedding before ranking and truncation**. Scores and ordering inside that candidate pool therefore match the flat metric; quantization and approximate navigation can still affect *beam recall* — which neighbours enter the pool — but not a returned node's reported score.

### The `.vg` SST body format and decoded-index cache

A `.vg` body is self-contained so a top-k needs exactly one object fetch. The current layout is 8-byte magic `MAGIC = b"NAMIVG04"` followed by a bincode-serialized `VectorGraphBody { dim, metric, ids, storage, graph, filter_postings }`. `VectorStorage` is `F32(Vec<Vec<f32>>)` or `Int8 { codes, scales }`; `ids`, `storage`, and `graph.adjacency` are parallel per vector ordinal. `filter_postings` maps indexed String/Bool properties and typed ScalarV1 keys to an adaptive `Sparse(Vec<u32>)` or `Dense(Vec<u64>)` ordinal posting. v3 (`NAMIVG03`, no postings) has an exact legacy wire struct and remains readable; its vector search works normally and filtered queries report native filtering unsupported.

**There is no checksum**: `VectorGraphIndex::decode` validates magic, metric, parallel lengths, graph entry, int8 geometry, sorted/in-range sparse postings, and dense bitmap bounds. `build_body` returns `Ok(None)` when fewer than 2 members exist, validates `desc.dim`, and — **for cosine only** — excludes all-zero vectors so the indexed corpus matches the flat scan's NULL-drop; postings are remapped across that exclusion. Any decode error is treated as “index absent” and falls back exactly.

`VectorGraphIndex::search(query, k, ef)` navigates the graph for up to `ef` candidates in the navigation space (clamps `ef = ef.max(k)`), then reranks: f32 indexes recompute the true metric in f64 via `metric_score` from the original vectors; int8 uses `1.0 − dist` (the quantized cosine). Results sort by `higher_is_better() = !Euclidean`. Because decoding deserializes every vector plus the full adjacency *and* clones the vectors into the navigation space, the storage layer caches the decoded `Arc<VectorGraphIndex>` process-wide, keyed by absolute SST path, in the deeply weighted `SstCache.search_indexes` Foyer pool shared with `.ft` (`crates/namidb-storage/src/cache.rs`; fetched in `read.rs::fetch_vector_index`). SSTs are immutable per UUIDv7-keyed path, so cached indexes never go stale; superseded paths are pruned. Without it, every KNN and each widening round would pay O(index size).
...
**Filtered ANN (RFC-032, embedded ordinal postings + adaptive widening).** The procedure surface accepts `filter: {property: value}` (equality), a list value (`IN`), or comparison maps. Natural `MATCH … WHERE d.prop = $value` / `d.prop IN $values … ORDER BY <vector metric> LIMIT k` plans extract the same lossless String/Bool groups at execution time; the optimizer consumes only non-unique posting seeks, while unique-property and `elementId` filters remain exact point-lookup + `TopN` plans. During the same authoritative winner-stream merge that rebuilds a vector graph, every indexed String/Bool property is harvested as complete vector-ordinal postings. Postings switch between sorted `u32` ordinals and `u64` bitmaps at the N/64 crossover; alternatives OR and conjuncts AND directly as bitmap words. Rejected ordinals remain graph-navigation waypoints but are removed before metric reranking and before `k`, so they consume neither result slots nor node hydrations. `vector_filter_bitmap_searches()` exposes a monotonic counter for this native path; vector-enabled servers publish it as `namidb_vector_filter_bitmap_searches_total`.

The build is bounded by `NAMIDB_VECTOR_FILTER_MAX_DISTINCT` (default 4096 per property) and `NAMIDB_VECTOR_FILTER_MAX_BYTES` (default 64 MiB per body, measured over the adaptive payload plus conservative map overhead). Crossing either cap drops the whole property—never a truncated posting, because presence means complete coverage. A missing property therefore returns “unsupported” and keeps the residual fallback, while a missing value inside a present property is an exact empty result. v4 gets first refusal without opening equality sidecars. Only when it applies no group does the executor lazily try a complete, LWW-confirmed sidecar set capped by `NAMIDB_VECTOR_FILTER_ID_CANDIDATE_CAP` (default 8192); oversized/high-cardinality sets are rejected before candidate hydration. Numeric operands remain residual because Cypher equates `1` and `1.0` while physical typed keys differ.

A residual source `WHERE`, an unindexed/range procedure predicate, or the part of a mixed filter not expressible as equality remains in the historically named `post_filter` field, but is semantically pre-k. Selectivity is still correctness-safe: the executor widens geometrically before the O(n) exact fallback, with `OVERFETCH_BASE = 8`, `WIDEN_GROWTH = 4`, `MAX_WIDEN_ROUNDS = 4` (multipliers **8 → 32 → 128 → 512**). In the native path `ef` grows over the unfiltered navigation graph while `k` counts only eligible hits; exhaustion is proven by `ef >= point_count`, not by a short filtered result. In the residual-only path each round fetches `kprime = max(k, k·mult + |delta_ids|)`, merges + evaluates, and a short unfiltered result proves exhaustion. Either way, fewer than `k` survivors after widening returns `Ok(None)` to trigger the exact flat scan (the ground truth). A post-k `Filter` is never folded into this field. With **no** source filter there is one round. A per-candidate `check_deadline()` keeps widened retrieval interruptible.

**Beam-width surface.** The procedures (`search.vector`, `search.hybrid`, `db.index.vector.queryNodes`) take a first-class `ef`; the natural/operator form reads a namespaced, explicitly non-stable parameter `$__vector_ef` threaded into the same `ef_search` slot. It is clamped to at least the requested candidate count; a value below the default 64 may trade approximate recall for latency, while selective filters still retain their exact under-fill fallback. RFC-036 proposes replacing it with a stable `OPTIONS { ef: … }` clause.
...
`bm25_ranked` is the retrieval core shared by `search.bm25` and the sparse leg of `search.hybrid`. It parses the query once, consults the index via `text_search`, and on `None` scans the label. `doc_text` forms each document by joining the configured string properties with a space; a node carrying none of them is not in the corpus. The exact flat path computes N, `total_len`, per-fixed-term `df` and prefix-matched-term `df` in pass one, then applies an optional hybrid predicate and maintains the top-k in pass two. Filtering therefore never changes corpus-wide BM25 statistics. Prefix expansion re-uses the same `BTreeMap::range` + `take_while` + `PREFIX_EXPANSION_LIMIT` pick over the discovered vocabulary, so the flat path expands to the same lexicographic head as the index. Scored terms are accumulated in a `BTreeMap` so summation order matches the index. Both passes poll `namidb_storage::cancel::deadline_exceeded()` every 4096 documents. With an authoritative FTS body, `bm25_ranked_filtered` hydrates only newly exposed slices while widening the ranked prefix up to `NAMIDB_HYBRID_TEXT_FILTER_CANDIDATE_CAP`; exhaustion is exact and reaching the cap falls back to the two-pass scorer, so the cap affects cost rather than recall.
...
`namidb-bolt` implements Bolt 4.4/5.0/5.4 with no dependency on the server crate (RFC-022). The PackStream codec (`codec.rs`) is a marker-by-marker translation with a critical pre-auth defense: `decode_inner` threads a `depth` counter and rejects nesting beyond `MAX_NESTING_DEPTH = 128` with a clean `NestingTooDeep` failure, because a malformed message of nested `List`/`Map`/`Struct` markers could otherwise drive the recursive decoder into a non-unwindable worker-thread stack overflow that aborts the process — reachable before authentication. Every declared string/byte length and container cardinality is checked against the active message ceiling before allocation; the whole decoded tree additionally shares a heap budget of `64 KiB + 8 × actual wire bytes`, including conservative `Vec` slots, map nodes and key payloads, so many individually legal nested containers cannot amplify one bounded frame into unbounded heap. Message bodies keep a strict `PRE_AUTH_MESSAGE_BYTES = 64 KiB` cap until authentication, then use the server-resolved `NAMIDB_BOLT_MAX_MESSAGE_BYTES` ceiling (default `DEFAULT_POST_AUTH_MESSAGE_BYTES = 64 MiB`). The setting is resolved once at boot and copied into each session; it never weakens the pre-auth cap. Authenticated frames use a two-phase process-wide lease configured by `NAMIDB_BOLT_MEMORY_BUDGET_BYTES` (half of `NAMIDB_MEMORY_MAX_BYTES`, or ~1 GiB without that governor): an incomplete frame grows at `64 KiB + 2 × wire bytes` for retained `Vec`/chunk capacity, then a terminated data frame atomically upgrades to `64 KiB + 16 × wire bytes` before decode. Every acquisition is fail-fast, so no task waits while retaining partial permits. The upgraded lease follows the body through decode, parameter conversion, execution and RUN prefetch. Before decode, the server also uses a CAS-accounted RAII reservation for measured RSS plus every concurrent projected working set, retained through handling; ordinary backend admission remains at execution time. A fixed deadline from the first partial-frame byte (`NAMIDB_BOLT_PARTIAL_MESSAGE_TIMEOUT`, 120 s) releases a slow client's bounded raw lease; idle connections hold neither a lease nor this deadline. Small bounded PULL/DISCARD/COMMIT/ROLLBACK/RESET/GOODBYE/LOGOFF frames bypass data admission so pressure can always be relieved. RUN transport prefetch is additionally capped at 16 messages and one post-auth raw-body budget, including a partial frame. Impossible-size frames receive an invalid-request `FAILURE`; temporary shared-budget contention receives a retryable transient `FAILURE`, then that connection closes cleanly.
...
- **Parity and recall tests**, the load-bearing class for index semantics (§1.3). The index and flat paths run against the same committed state: BM25 and factorized execution are asserted equal; vector fixtures assert score/filter/freshness parity for reached candidates and separate clustered-data tests enforce recall floors against brute-force neighbours. A parity test is only meaningful if it confirms the index path was *actually taken*, so these tests additionally assert that the storage layer returned an index answer (rather than silently falling back), or that the optimized plan contains the index operator.
...
A separate track (`vector_recall.rs`) isolates the error introduced by int8 quantization *alone* — the arithmetic of encoding, dequantizing, and the asymmetric quantized distance — from the graph-navigation error, so the two sources of approximation can be attributed independently. This matters because the engine rescan-corrects quantized scores with the exact f32 metric on the candidate pool (§6): reported scores and ordering inside the reached pool are exact, while quantization and graph navigation affect which neighbours are reached, i.e. recall.
...
| **Freshness gate** | The mechanism ensuring an index observes the same committed corpus as a scan — either delta-union or gate-and-fall-back. ANN neighbour membership remains approximate. |
...
Read from the source at 2.0.6. Runtime-tunable values give their environment variable.
...
| Exact node locator | suffix; paged magic; page size | `.nloc2`; `NAMIPG02`; 4 KiB |
| Bloom | bits/key; omit threshold; block size | 10 (~1% FPR); 256 KiB; 256-bit |
| Edge SST | fence-index threshold; fence stride; partner skew threshold; header magic | 65,536 keys; 256; 1,024; `TGEDGE\0\0` |
| Format magic | vector `.vg`; text `.ft`; bloom | `NAMIVG04` (reads v3); `NAMIFT02`; `TGBLOOM\0` |
| Compaction | L1 byte budget; per-level growth; output merge chunk; input cursor batch | ~8 MiB; 10×; 16 Ki rows (`NAMIDB_COMPACTION_MERGE_CHUNK_ROWS`); 64 rows |
...
| Bolt | max nesting; pre/post-auth body; decoded heap; global partial/decoded admission; partial frame; RUN prefetch; connections; max tx lifetime | 128; 64 KiB / 64 MiB (`NAMIDB_BOLT_MAX_MESSAGE_BYTES`); 64 KiB + 8× wire bytes; 64 KiB + 2× / 16× wire bytes (`NAMIDB_BOLT_MEMORY_BUDGET_BYTES`); 120 s (`NAMIDB_BOLT_PARTIAL_MESSAGE_TIMEOUT`); 16 messages and ≤ post-auth body cap; 1024 (`NAMIDB_BOLT_MAX_CONNECTIONS`); 300 s |
...
<sub>NamiDB is developed by NamiDB, Inc. (Delaware, USA) and licensed under the Business Source License 1.1. This report describes version 2.0.6. Corrections and questions: the source is the authority; every claim here is traceable to a file path cited inline.</sub>
