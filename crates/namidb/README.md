...
Public façade crate for [NamiDB](https://github.com/namidb/namidb) —
the graph database, native to the cloud.

This crate is the **stable umbrella API**. It re-exports the most
commonly used types from
[`namidb-core`](../namidb-core/),
[`namidb-storage`](../namidb-storage/),
[`namidb-graph`](../namidb-graph/) and
[`namidb-query`](../namidb-query/) so that downstream code can depend
on a single `namidb = "0.1"` line in `Cargo.toml`.

If you are embedding NamiDB into a Rust application, **start here**.
